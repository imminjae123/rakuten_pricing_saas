...
- **Redis serves three distinct roles**: (1) Rakuten API rate-limit token bucket, (2) Celery/RQ broker for LINE notification jobs, (3) cache for tenant policy metadata — use separate Redis DB indices (0/1/2) to avoid key collisions
