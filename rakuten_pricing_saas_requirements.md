# B2B SaaS 要件定義書：楽天市場 競合価格モニタリング＆自動価格対応ソリューション

## 1. プロジェクト概要
* **プロジェクト名:** Rakuten Dynamic Pricing Tracker (仮称)
* **目的:** 日本の楽天市場に出店しているセラー（テナント）が、競合店舗の価格変動をリアルタイムまたはバッチ処理でモニタリングし、事前定義したビジネスルール（Rule）に従って自社商品の最適販売価格を自動算出し対応できるように支援するB2B SaaSプラットフォーム。
* **学習目標 (個人プロジェクトの観点):**
...
* **F-1. 店舗 (Tenant) 登録:** セラーは自社の楽天ショップ情報 (Shop Code等) を登録してテナントを作成する。
...
### 3.2. 商品マッピングおよび原価管理
* **F-3. 自社商品登録:** モニ터リング対象の自社商品情報、**原価 (Cost)**、**最小保証マージン額/率**を登録する。(※該当情報はテナント内でのみ厳重に保護される)
* **F-4. 競合商品マッピング:** 自社商品1件に対し、モニタリング対象の競合商品 (楽天 Item Code) を1:Nでマッピングする。
...
* **F-7. 定期価格モニタリング:** システムは1時間/1日周期で楽天APIを呼び出し、登録された競合商品の現在価格を更新する。
* **F-8. Rate Limiting 防御ロジック:** 楽天APIの1秒あたりの呼び出し制限を超えないよう、トークンバケット (Token Bucket) や Sleep ベースの Throttling 処理をパイプラインに実装する。
...
* `Competitor_Products`: 楽天市場でモニタリングする他社商品情報
